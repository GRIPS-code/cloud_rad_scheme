...
def read_from_pade(file,Re,shape):
    ds = nc.Dataset(file)
    band = np.zeros(2,ds.Dimensions.Band)
    ...
    r_ref      = ds.Effective_Radius_Ref

    for iRe in range(len(ds.Dimensions.Re_range)):
    ...
    if nargin==2:
        ...
        ext = compute_from_pade_2d(iRe, Re-r_ref(iRe),squeeze(pade_ext_p(:,:,:)),squeeze(pade_ext_q(:,:,:)),shape,shape_ref,squeeze(pade_ext_gamma_p(:,:,:)),squeeze(pade_ext_gamma_q(:,:,:)),squeeze(pade_ext_gamma_a(:,:,:)),squeeze(pade_ext_gamma_b(:,:,:)))
        ssa = compute_from_pade_2d(iRe, Re-r_ref(iRe),squeeze(pade_ssa_p(:,:,:)),squeeze(pade_ssa_q(:,:,:)),shape,shape_ref,squeeze(pade_ssa_gamma_p(:,:,:)),squeeze(pade_ssa_gamma_q(:,:,:)),squeeze(pade_ssa_gamma_a(:,:,:)),squeeze(pade_ssa_gamma_b(:,:,:)))
        asy = compute_from_pade_2d(iRe, Re-r_ref(iRe),squeeze(pade_asy_p(:,:,:)),squeeze(pade_asy_q(:,:,:)),shape,shape_ref,squeeze(pade_asy_gamma_p(:,:,:)),squeeze(pade_asy_gamma_q(:,:,:)),squeeze(pade_asy_gamma_a(:,:,:)),squeeze(pade_asy_gamma_b(:,:,:)))
